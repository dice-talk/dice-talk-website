...
export default function Sidebar() {
  ...
  const isSubItemActive = (subItemPath: string, currentPath: string): boolean => {
    // 1. Exact match
    ...
    if (subItemPath === '/notices') {
      return currentPath.startsWith('/notices/') && 
    ...
    if (subItemPath === '/qnalist') {
      ...
      return currentPath.startsWith('/qna/') || currentPath.startsWith('/qnalist/');
    }
  ...
  useEffect(() => {
    ...
    for (const item of menuItems) {
      if (item.subItems?.some(sub => isSubItemActive(sub.path, location.pathname))) {
  ...
  const toggleMenu = (menuName: string) => {
    ...
    if (openMenu === menuName) {
      ...
      if (mouseLeaveTimerRef.current) { // 이 메뉴에 대한 mouseLeave 타이머가 있었다면 취소
        clearTimeout(mouseLeaveTimerRef.current);
        mouseLeaveTimerRef.current = null;
      }
    }
    ...
    <aside className="w-64 min-h-screen bg-gradient-to-b from-blue-300 to-purple-300 text-white shadow-xl z-20 flex flex-col items-center pt-8">
      ...
      <nav className="w-full flex flex-col space-y-2 mt-2">
        {menuItems.map((item) => {
          ...
          const isEffectivelyOpen = (hoveredMenu === item.name) || (hoveredMenu === null && openMenu === item.name);
          const isParentActive = item.subItems?.some(sub => isSubItemActive(sub.path, location.pathname));
          
          const transitionConfig = shouldAnimateTransition
            ? { duration: MENU_ANIMATION_DURATION_SECONDS, ease: [0.25, 0.8, 0.25, 1] }
          ...
          return item.subItems ? (
            ...
            >
                {item.name}
               {isEffectivelyOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </button>
