...
export default function ChatRoomManagementPage() {
  ...
  const fetchAndSetChatRooms = useCallback(
    async (
      ...
      try {
        const params: Parameters<typeof getChatRooms>[0] = {
          ...
          size: size,
          themeName:
            appliedFilters.themeName === "ALL"
              ? undefined
              : appliedFilters.themeName,
          roomStatus:
            appliedFilters.status === "ALL"
              ? undefined
              : (appliedFilters.status as ChatRoomStatusEnum),
          roomType:
            appliedFilters.roomType === "ALL"
              ? undefined
              : (appliedFilters.roomType as ChatRoomTypeEnum),
          chatRoomId: appliedFilters.roomId
            ? Number(appliedFilters.roomId)
            : undefined,
          sort: sortValue, // DESC/ASC만 전달
        };
        const response: MultiResponse<ChatRoomMultiResponseDto> =
    ...
    [appliedFilters, currentPage, sortValue]
  );
  ...
  const handleForceCloseRoom = async (chatRoomId: number) => {
    if (
      window.confirm(`채팅방 ID ${chatRoomId}을(를) 강제 종료하시겠습니까?`)
    ) {
      try {
  ...
  const columns: ColumnDefinition<ChatRoomTableItem>[] = [
    ...
    },
    {
      ...
      header: "방 유형",
      accessor: (item) =>
        getChatRoomTypeLabel(item.roomType as ChatRoomTypeEnum),
      headerClassName: "w-[15%]",
    },
    {
      key: "themeName",
      header: "테마",
      accessor: (item) => item.themeName || "-",
      headerClassName: "w-[15%]",
    },
    {
      key: "roomStatus",
      ...
      headerClassName: "w-[10%]",
      cellRenderer: (item) => (
        <StatusBadge status={item.roomStatus} type="chatRoom" />
      ),
    },
    {
      key: "createdAt",
      ...
      cellRenderer: (item) => (
        <div className="space-x-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => handleViewDetails(item.chatRoomId)}
          >
            조회
          </Button>
          ...
          {item.roomStatus !== ChatRoomStatusEnum.ROOM_DEACTIVE && (
            <Button
              variant="destructive"
              size="sm"
              onClick={() => handleForceCloseRoom(item.chatRoomId)}
            >
              강제종료
            </Button>
          )}
  ...
  const chatRoomSortOptions = [
    { value: "DESC", label: "생성일 (최신순)" },
    { value: "ASC", label: "생성일 (오래된순)" },
  ];
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 bg-slate-50 p-6 md:p-8 rounded-tl-xl overflow-y-auto">
          ...
          <ChatRoomFilterSection
            themeNameFilter={themeNameFilter}
            onThemeNameFilterChange={setThemeNameFilter} // props 이름 변경
          ...
          />
          {isLoading && (
            <p className="text-center py-4">채팅방 목록을 불러오는 중...</p>
          )}
          {error && (
